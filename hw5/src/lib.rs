    ...
    V: Debug,
{
        ...
        if self.has_key(key) == false{
            ...
            loop {
                match &mut self.inner {
                    ...
                    Some(x) => {
                        if x.key == *key {
                            return Some(&mut x.val);
                        } else if x.key > *key {
                            return x.lt.get_mut(key);
                        } else{
                            return x.rt.get_mut(key);
                        }
                    }
                };
            }
    ...
    pub fn insert(&mut self, key: K, val: V) -> Option<V> {
        ...
        match &mut self.inner{
            None => {
                ...
                self.inner = Some(Box::new(n));
                return None;
            }
            Some(x) => {
                if x.key == key {
                    let cVal = mem::replace(&mut x.val,val);
                    return Some(cVal)
                } else if x.key > key {
                    x.size = x.size+inc;
                    return x.lt.insert(key,val);
                } else{
                    x.size = x.size+inc;
                    return x.rt.insert(key,val);
                }
            }
        }
    }
                ...
                match &mut self.inner{
                    ...
                    Some(x) => {
                        if x.key == y.key {
                            ...
                            x.rt.insert_tree(other)
                        } else{
                            x.lt.insert_tree(other)
                        }
                    }
                }
            }
    ...
    pub fn remove(&mut self, key: K) -> Option<V> {
        ...
        match &mut self.inner {
            ...
            Some(x) => {
                if x.key == key {
                    let mut note = self.inner.take().unwrap();
                    let rVal = note.val;
                    note.rt.insert_tree(&mut note.lt);
                    self.inner = note.rt.inner;
                    return Some(rVal);
                } else if x.key > key {
                    x.size = x.size-inc;
                    return x.lt.remove(key);
                } else{
                    x.size = x.size-inc;
                    return x.rt.remove(key);
                }
            }
        }
    }
...
impl<K, V> IntoIter<K, V> {
    ...
    fn new(tree: TreeMap<K, V>) -> Self {
        let mut cur = IntoIter{
            next_nodes: Vec::new(),
            current_val: None
        };
        cur.descend_left(tree);
        return cur;
    }
    ...
    fn descend_left(&mut self, tree: TreeMap<K, V>) {
        match tree.inner{
            None => match self.next_nodes.pop() {
                        Some(x) => {
                            match x {
                                Next::Item(i) => self.current_val = Some(i),
                                Next::Tree(t) => self.descend_left(t),
                            }
                        }
                        None => return,
                    }
            Some(cur) => {
                let rt = Next::Tree(cur.rt);
                self.next_nodes.push(rt);
                match &cur.lt.inner{
                    None => self.current_val = Some((cur.key,cur.val)),
                    Some(l) => {
                        let p = Next::Item((cur.key,cur.val));
                        self.next_nodes.push(p);
                        self.descend_left(cur.lt);
                    }
                }
            }
        }
...
impl<'a, K, V> IterMut<'a, K, V> {
    ...
    fn descend_left(&mut self, tree: &'a mut TreeMap<K, V>) {
        match &mut tree.inner{
            ...
            Some(cur) => {
                let rt = Next::Tree(&mut cur.rt);
                self.next_nodes.push(rt);
                match &cur.lt.inner{
                    None => self.current_val = Some((&cur.key,&mut cur.val)),
                    Some(l) => {
                        let p = Next::Item((&cur.key,&mut cur.val));
                        self.next_nodes.push(p);
                        self.descend_left(&mut cur.lt);
                    }
                }
            }
        }
